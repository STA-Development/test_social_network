	...
	const handlePostSubmit = async (e: FormEvent): Promise<void> => {
		...
		if(validationResult){
			...
			try {
				...
				setButtonLoading(false);
				setFilePreview('');
				ToastNotifyError('Something went wrong check if all inputs are filled and image type is correct(jpg/png) ');
			}
		}
		else{
			ToastNotifyError();
		...
		<>
			...
			<div className="w-full px-3 py-8">
				...
				<div className="w-full flex justify-center">
					<form onSubmit={(e: FormEvent) => handlePostSubmit(e)} className="w-7/12 min-h-80 bg-gray-light shadow-2xl rounded p-3" >
